...
function App() {
		...
		numbers
			...
			.map((number, index) => {
				...
				return (
					<div
						key={index}
						className="font-bold text-white rounded-full bg-purple-500 text-lg flex items-center justify-center text-center font-mono"
						style={{
							width: `40px`,
							height: `40px`,
						}}
	...
	return (
		<div className="bg-orange-100 p-4 md:p-16 rounded-xl ease-in duration-300 drop-shadow-md hover:drop-shadow-2xl">
			<h1 className="text-rose-600 text-3xl md:text-6xl font-bold mb-8">
				Lottery 7
			</h1>

			<div className="flex flex-wrap justify-center my-4 w-full gap-2 md:gap-6">
				{loading ? (
					<p className="text-blue-600 font-bold">I am thinking ...</p>
				) : (
					numbersList
				)}
			</div>
			...
			{loading ? null : (
				<div className="flex flex-col md:flex-row gap-4 mt-8 justify-center">
					<button
						...
						onClick={() => handleClick()} rel="noreferrer"
					>
						Click to Pick
					</button>
			...
			<a
        className="block mt-8 text-sm text-slate-400"
				href="https://github.com/johnsonta87/lottery-7"
				target="_blank"
				rel="noreferrer"
			>
        Github
